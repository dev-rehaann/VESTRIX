...
| 4. Forensic log | Append timestamp, node, raw/feature hashes, model metadata, class, confidence, explanation or transport metadata, chain links, and signature to canonical JSONL. | Logger and independent Rust verifier are implemented. The default adapter durably signs and appends each accepted collector event using the documented collector-ingest profile. Rejected attempts remain in collector operational logs rather than the signed chain; a forensic append failure causes the collector to reject the event. | Local or operator-selected append-only JSONL store, backups, and any exported chain tips or timestamp proofs. | Evidence custodians, incident responders, legal teams, and system administrators as authorized by the operator. |
