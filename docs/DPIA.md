...
| 4. Forensic log | Append timestamp, node, raw/feature hashes, model metadata, class, confidence, SHAP snapshot, chain links, and signature to canonical JSONL. | Logger and independent Rust verifier are implemented. The default collector-to-forensics adapter remains a no-op. | Local or operator-selected append-only JSONL store, backups, and any exported chain tips or timestamp proofs. | Evidence custodians, incident responders, legal teams, and system administrators as authorized by the operator. |
...
- an end-to-end dispatcher connecting collector, ML, forensic, and SOC stages;
